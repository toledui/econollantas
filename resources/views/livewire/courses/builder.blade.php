<div class="py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto" x-data="{ currentTab: @entangle('currentTab') }">
    ...
    <div class="mb-8">
        <nav class="flex items-center gap-2 text-sm text-slate-500 mb-3">
            ...
            <span class="text-slate-900 font-bold">{{ $isNew ? ' Nuevo Curso' : 'Editor de Curso'}}</span>
        </nav>
...
" x-on:trix-change="content = $event.target.value" x-on:trix-focus="isFocused = true"
                                x-on:trix-blur="isFocused = false" class="mt-1">
                                <input id="description_input" type="hidden" name="content">
                                <trix-editor x-ref="trix" input="description_input"
                                    ...
                                    placeholder="Detalla los objetivos y contenido del curso..."></trix-editor>
                            </div>
                                ...
                                <label for="dropzone-file"
                                    ...
                                    @else
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                            ...
                                            <p class="mb-1 text-sm text-slate-500"><span class="font-bold">Haz clic para
            ...
            <div x-show="currentTab ==='assessments'" x-transition.opacity.duration.300ms class="space-y-6"
                ...
                <div class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200">
                    ...
                    @else
                        <div class="py-12 flex flex-col items-center justify-center text-center">
                            <div class="size-16 bg-slate-100 rounded-full flex items-center justify-center mb-4">
                                <span class="material-symbols-outlined text-3xl text-slate-400">fact_check</span>
                            </div>
                            <h3 class="text-base font-bold text-slate-900 mb-1">El curso aún no tiene
                                evaluaciones</h3>
                        ...
                        <div class="space-y-3">
                            @foreach($course->assignments->sortByDesc('assigned_at') as $assign)
                                ...
                                ">
                                                            ...
                                                            <button x-on:click="confirmRemoveAssignment({{ $assign->id }})"
                            ...
                            <table class="w-full text-sm text-left">
                                <thead class="bg-slate-50 border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th scope="col"
                                            class="px-6 py-4 font-bold text-slate-500 uppercase text-xs tracking-wider w-full">
                                            Usuario
                                        </th>
                                        <th scope="col"
                                            class="px-6 py-4 font-bold text-slate-500 uppercase text-xs tracking-wider whitespace-nowrap">
                                            Origen
                                        </th>
                                        <th scope="col"
                                            class="px-6 py-4 font-bold text-center text-slate-500 uppercase text-xs tracking-wider whitespace-nowrap">
                                            Estado
                                        </th>
                                        <th scope="col"
                                            class="px-6 py-4 font-bold text-right text-slate-500 uppercase text-xs tracking-wider whitespace-nowrap">
                                            Acciones
                                        </th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr class="hover:bg-slate-50/50 transition-colors">
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    {{-- Avatar optimizado --}}
                                                    <div
                                                        class="h-10 w-10 rounded-full overflow-hidden shrink-0 border border-slate-200 shadow-sm">
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                @if($student->status === 'not_started')
                                                    <span
                                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-600">
                                                        Pendiente
                                                    </span>
                                                @elseif($student->status === 'completed')
                                                    <span
                                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700">
                                                        Completado
                                                    </span>
                                                @elseif($student->status === 'revoked')
                                                    <span
                                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-red-100 text-red-700">
                                                        Excluido
                                                    </span>
                                                @else
                                                    <span
                                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-700">
                                                        En Progreso
                                                    </span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 text-right">
                                                ...
                                                @else
                                                    <button x-on:click="confirmRemoveUser({{ $student->user_id }})"
                                                        class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all"
                                                        title="Remover / Excluir">
                                                        <span
                                                            class="material-symbols-outlined text-[20px] block">person_remove</span>
                                                    </button>
                                                @endif
                                            </td>
                                        </tr>
...
" x-on:trix-change="content = $event.target.value" class="mt-1">
                        <input id="lesson_description_input" type="hidden">
                        <trix-editor x-ref="trix_lesson" input="lesson_description_input"
                            ...
                            placeholder="Detalla de qué trata esta lección..."></trix-editor>
                    </div>
        ...
        <form wire:submit="saveAssessment" class="p-6">
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-tight mb-1">Tipo
                            de Evaluación</label>
    ...
    <x-modal wire:model="showContentModal" maxWidth="5xl">
        <div class="p-6 md:p-8 flex flex-col" style="min-height: 80vh!important;">
            <div class="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
                ...
                <button wire:click="closeContentModal" type="button"
    ...
    <x-modal wire:model="showAssessmentQuestionModal" maxWidth="5xl">
        <div class="p-6 md:p-8 flex flex-col" style="min-height: 90vh!important;">
            <div class="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
                ...
                <button wire:click="closeAssessmentQuestionModal" type="button"
                    class="text-slate-400 hover:text-slate-600 transition-colors">
                    <span class="material-symbols-outlined">close</span>
                </button>
            </div>
    ...
    <script>
        ...
        function confirmRemoveAssignment(id) {
            Swal.fire({
                ...
                text: "¿Seguro que deseas eliminar el registro de esta asignación y revocarlo a todos los usuarios que aún no comienzan el curso?",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#ff4b4b',
                cancelButtonColor: '#6b7280',
        ...
        function confirmRemoveUser(id) {
            Swal.fire({
                ...
                text: "Si fue asignado por departamento, no volverá a aparecer automáticamente.",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#ff4b4b',
                cancelButtonColor: '#6b7280',
